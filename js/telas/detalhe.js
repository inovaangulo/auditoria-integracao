...
/**
 * Select generico com opcoes fixas (CLIENTES_CONHECIDOS, CARGOS_CONHECIDOS
 * etc.) pra evitar grafia diferente pro mesmo valor. Se o valor atual nao
 * bater com nenhuma opcao (dado antigo, digitado a mao antes da lista
 * existir), mostra uma opcao extra pra nao sumir com o dado.
 */
function campoSelectComOpcoesFixas(rotulo, campo, opcoesConhecidas) {
  const atual = rascunho[campo] || '';
  const opcoes = [...opcoesConhecidas];
  if (atual && !opcoes.includes(atual)) opcoes.push(atual);

  const select = el('select', { 'data-campo': campo });
  select.append(el('option', { value: '', texto: 'Selecione…', selected: !atual }));
  for (const c of opcoes) {
    const texto = !opcoesConhecidas.includes(c) ? `${c} (fora da lista)` : c;
    select.append(el('option', { value: c, texto, selected: atual === c }));
  }
  select.addEventListener('change', () => { select.classList.remove('erro'); definir(campo, select.value); });
  return el('label', { class: 'campo' }, [el('span', { texto: rotulo }), select]);
}

function campoClienteAtual() {
  return campoSelectComOpcoesFixas('Cliente atual', 'Cliente atual', CLIENTES_CONHECIDOS);
}
